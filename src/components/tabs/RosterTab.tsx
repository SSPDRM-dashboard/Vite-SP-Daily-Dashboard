    ...
    <div className="animate-in fade-in duration-300">
      <div className="bg-gradient-to-br from-[#001f5c] to-[#0a3fa8] rounded-2xl p-4 md:p-6 flex items-center justify-between mb-5 text-white shadow-md print:hidden">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            <div className="font-bold text-xl md:text-2xl text-white tracking-wide">Roster Penugasan</div>
            <div className="text-[13px] text-white/80 mt-0.5">Jadual penugasan bulanan anggota</div>
          </div>
        </div>
      </div>
      ...
      {!loading && !error && rosterData.length > 0 && (
        <div className="bg-white border border-slate-200 shadow-sm overflow-x-auto print:border-none print:shadow-none print:overflow-visible">
          ...
          <div className="min-w-[1200px] text-black font-sans text-xs print-container">
            ...
            <table className="w-full border-collapse border border-black text-[10px] text-center">
              <thead>
                <tr>
                  ...
                  <th className="border border-black p-1" colSpan={daysInMonth}>JUMLAH BERTUGAS PADA TARIKH TERSEBUT</th>
                </tr>
                <tr>
                  {daysArray.map(day => (
                    <th 
                      key={day} 
                      className={`border border-black p-1 w-[26px] min-w-[26px] max-w-[26px] ${isWeekend(day) ? 'bg-blue-300' : ''}`}
                    >
                      {day}
                    </th>
                  ))}
                  ...
                  <React.Fragment key={`row-${idx}`}>
                    ...
                    <tr>
                      ...
                      <td className="border border-black p-1 font-bold text-left px-2" rowSpan={2}>
                        <div className="line-clamp-3 text-[8px] leading-tight break-words whitespace-normal">
                    ...
                    <tr>
                      {daysArray.map(day => {
                        ...
                        return (
                          <td 
                            key={`masa-${day}`} 
                            className={`border border-black p-0.5 text-[6px] leading-[1.1] font-bold w-[26px] min-w-[26px] max-w-[26px] ${isWeekend(day) ? 'bg-blue-300' : ''}`}
                          >
